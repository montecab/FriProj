...
public class piConnectionManagerExampleUI : MonoBehaviour {
	...
	private float velLinear = 0;
	private float velAngular = 0;
	...
	void OnGUI() {
		...
		if (picm.BotsInState(PI.BotConnectionState.CONNECTED).Count > 0 && showExampleCommands) {
			...
			{	// linear and angular velocity
				float dLinear = 5.0f;
				float dAngular = Mathf.Deg2Rad * 90.0f;
				// linearVelocity
				WN = 3;
				wn = 0;
				bool sendLinearAngular = false;
				if (GUI.Button(new Rect(cursor.x + buttonSize.x / WN * (wn++), cursor.y, buttonSize.x / WN, buttonSize.y), "lin. -")) {
					velLinear -= dLinear;
					sendLinearAngular = true;
				}
				if (GUI.Button(new Rect(cursor.x + buttonSize.x / WN * (wn++), cursor.y, buttonSize.x / WN, buttonSize.y), "lin. 0")) {
					velLinear = 0;
					sendLinearAngular = true;
				}
				if (GUI.Button(new Rect(cursor.x + buttonSize.x / WN * (wn++), cursor.y, buttonSize.x / WN, buttonSize.y), "lin. +")) {
				...
				WN = 3;
				wn = 0;
				if (GUI.Button(new Rect(cursor.x + buttonSize.x / WN * (wn++), cursor.y, buttonSize.x / WN, buttonSize.y), "ang. -")) {
					velAngular -= dAngular;
					sendLinearAngular = true;
				}
				if (GUI.Button(new Rect(cursor.x + buttonSize.x / WN * (wn++), cursor.y, buttonSize.x / WN, buttonSize.y), "ang. 0")) {
					velAngular = 0;
					sendLinearAngular = true;
				}
				if (GUI.Button(new Rect(cursor.x + buttonSize.x / WN * (wn++), cursor.y, buttonSize.x / WN, buttonSize.y), "ang. +")) {
				...
				cursor.y += buttonSize.y;
				if (sendLinearAngular) {
					foreach (piBotBo bot in picm.BotsInState(PI.BotConnectionState.CONNECTED)) {
						bot.cmd_bodyMotion(velLinear, velAngular);
					}
				}
			}
		...
		if (showSensors) {
			foreach (piBotBo bot in picm.BotsInState(PI.BotConnectionState.CONNECTED)) {
				...
				piBotComponentDistanceSensor distSens;
				distSens = bot.DistanceSensorFrontLeft;
				GUI.Label(new Rect(cursor.x + labelSize.x / 2 * 0, cursor.y, labelSize.x / 2, labelSize.y), "FLD:" + distSens.distance.ToString("0.0"));
				GUI.Label(new Rect(cursor.x + labelSize.x / 2 * 1, cursor.y, labelSize.x / 2, labelSize.y), "FLR:" + distSens .reflectance.ToString("0"));
				cursor.y += labelSize.y;
				distSens = bot.DistanceSensorFrontRight;
				GUI.Label(new Rect(cursor.x + labelSize.x / 2 * 0, cursor.y, labelSize.x / 2, labelSize.y), "FRD:" + distSens.distance.ToString("0.0"));
				GUI.Label(new Rect(cursor.x + labelSize.x / 2 * 1, cursor.y, labelSize.x / 2, labelSize.y), "FRR:" + distSens.reflectance.ToString("0"));
				cursor.y += labelSize.y;
				// distance sensor rear
				distSens = bot.DistanceSensorTail;
				GUI.Label(new Rect(cursor.x + labelSize.x / 2 * 0, cursor.y, labelSize.x / 2, labelSize.y), "T D:" + distSens.distance.ToString("0.0"));
				GUI.Label(new Rect(cursor.x + labelSize.x / 2 * 1, cursor.y, labelSize.x / 2, labelSize.y), "T R:" + distSens .reflectance.ToString("0"));
				cursor.y += labelSize.y;
				
				// sound sensor
				string s = bot.SoundSensor.eventId.ToString();
				GUI.Label(new Rect(cursor.x + labelSize.x / 2 * 0, cursor.y, labelSize.x / 1, labelSize.y), "S:" + s);
				cursor.y += labelSize.y;
			}
		}
	...
	void onButton_BotButton(piBotBase bot) {
		switch(bot.connectionState) {
			case PI.BotConnectionState.SELECTED:
