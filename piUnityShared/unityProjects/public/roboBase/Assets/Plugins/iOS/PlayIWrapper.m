...
+ (void)robot:(NSString*)robotUUID bodyLinearVelocity:(PIVelocity)linearVelocity angularVelocity:(PIVelocity)angularVelocity;
...
+ (void)robot:(NSString*)robotUUID setBrightness:(PIBrightness)brightness forLED:(uint32_t)component;
...
+ (void)robot:(NSString*)robotUUID stopAnimation:(PIUInteger)animId;
...
void PIRobot_bodyMotion(char* robotUUID, double linearVelocity, double angularVelocity) {
    [PlayIWrapper robot:[NSString stringWithUTF8String:robotUUID] bodyLinearVelocity:linearVelocity angularVelocity:angularVelocity];
}
...
void PIRobot_led(char* robotUUID, uint32_t component, uint8_t brightness) {
    [PlayIWrapper robot:[NSString stringWithUTF8String:robotUUID] setBrightness:brightness forLED:component];
}
...
void PIRobot_stopAnimation(char* robotUUID, uint32_t animId) {
    [PlayIWrapper robot:[NSString stringWithUTF8String:robotUUID] stopAnimation:animId];
}
...
+(void)startScan
{
    ...
    manager.delegate = (id<PIRobotManagerDelegate>)self; // set delegate to handle callback
    manager.scanPeriod = 2.0;
}
...
+(void)connect:(NSString *)robotUUID
{
    ...
    if (robot != nil) {
        [manager selectToConnectRobot:robot];
    }
}
...
+ (void) robot:(NSString*)robotUUID bodyLinearVelocity:(PIVelocity)linearVelocity angularVelocity:(PIVelocity)angularVelocity {
    PICommand *command = [PICommand new];
    PIComponentBodyMotionLinearAngular *component = [[PIComponentBodyMotionLinearAngular alloc] initWithLinear:linearVelocity angular:angularVelocity];
    [command setBodyMotionLinearAngular:component];
    [PlayIWrapper robot:robotUUID sendCommand:command];
}
...
+ (void) robot:(NSString*)robotUUID setBrightness:(PIBrightness)brightness forLED:(uint32_t)component
{
    PIComponentLightLED *led = [PIComponentLightLED new];
    led.brightness = brightness;
    PICommand *command = [PICommand new];
    [command setComponent:led forIndex:component];
    [PlayIWrapper robot:robotUUID sendCommand:command];
}
...
+ (void) robot:(NSString*)robotUUID stopAnimation:(uint32_t)animId
{
    [self setupStatics];
    ...
    if (sequence == nil) {
        NSLog(@"stopAnimation: unknown animation id: %@", animKey);
        return;
    }
    
    PIRobot* robot = [PlayIWrapper robotForUUID:robotUUID];
    if (robot != nil) {
        [robot stopCommandSequence:sequence];
    }
}

+ (void) robot:(NSString*)robotUUID sendRawPacket:(uint8_t*)bytes withCount:(uint32_t)count
{
    NSLog(@"ERROR: unsupported method: sendRawPacket");
    /*
    NSString* s = @"";
...
+ (void) manager:(PIRobotManager *)manager didConnectRobot:(PIRobot *)robot
{
    // note weirdness: 'self' is the class, not the instance.
    robot.delegate = (id<PIRobotDelegate>)self;
...
+ (void) manager:(PIRobotManager *)manager didSelectRobot:(PIRobot *)robot
{
    ...
    NSLog(@"selected %@, status: %d", robot.name, robot.connectionState);
    
    [PlayIWrapper sendUnityMessageWithMethodName:@"didSelectRobot" andRobot:robot];
}
...
+ (void) manager:(PIRobotManager *)manager didFailToConnectRobot:(PIRobot *)robot error:(NSError *)error
{
 ...
 + (NSDictionary*)robotToDict:(PIRobot*)robot
{
    ...
    NSString* robotName = (robot.name ? robot.name : @"(none)");
    
    [ret setObject:robotName  forKey:@"name"];
...
+ (NSDictionary*)robotStateToDict:(PIRobotState*)state
{
    ...
    for (id keyId in componentKeys) {
        ...
        switch (key) {
            ...
            {
                // don't pass on.
                NSLog(@"unhandled component: %d", key);
                componentDict = nil;
                break;
            }
                
            case COMPONENT_BODY_MOTION_LINEAR_ANGULAR:
            case COMPONENT_EYE_RING:
            case COMPONENT_LED_BUTTON_MAIN:
            case COMPONENT_LED_TAIL:
            case COMPONENT_MOTOR_HEAD_PAN:
            case COMPONENT_MOTOR_HEAD_TILT:
            case COMPONENT_RGB_CHEST:
            case COMPONENT_RGB_EYE:
            case COMPONENT_RGB_LEFT_EAR:
            case COMPONENT_RGB_RIGHT_EAR:
            case COMPONENT_SPEAKER:
            {
                // since contemporary API, we're getting sensors back as part of robot state.
                componentDict = nil;
                break;
            }
...
//              [componentDict setObject:[NSNumber numberWithDouble:component.reflectance  ] forKey:@"reflectance"  ];
...
//              [componentDict setObject:[NSNumber numberWithDouble:component.reflectance  ] forKey:@"reflectance"  ];
            ...
            case COMPONENT_SOUND_SENSOR:
            {
                PIComponentSoundSensor* component = (PIComponentSoundSensor*)[state componentFromKey:keyId];
                [componentDict setObject:[NSNumber numberWithUnsignedInt:component.volume   ] forKey:@"volume"   ];
                [componentDict setObject:[NSNumber numberWithDouble     :component.direction] forKey:@"direction"];
                [componentDict setObject:[NSNumber numberWithUnsignedInt:component.event    ] forKey:@"event"    ];
                break;
            }
...
+ (void) robot:(PIRobot *)robot didStopExecutingCommandSequence:(PICommandSequence *)sequence withResults:(NSDictionary *)results
{
    ...
    NSArray *sequenceIds = [loadedAnimations allKeysForObject:sequence];
    if (sequenceIds) {
        NSNumber *idKey = [sequenceIds firstObject];
        NSMutableDictionary* dict = [NSMutableDictionary new];
        [dict setObject:@"didStopCommandSequence"           forKey:@"method"];
        [dict setObject:[PlayIWrapper robotToDict:robot]    forKey:@"robot" ];
        [dict setObject:idKey forKey:@"sequenceId" ];
...
+ (void) robot:(PIRobot *)robot didFinishCommandSequence:(PICommandSequence *)sequence
{
    ...
    if (sequenceIds) {
        NSNumber *idKey = [sequenceIds firstObject];
        NSMutableDictionary* dict = [NSMutableDictionary new];
        [dict setObject:@"didFinishCommandSequence"           forKey:@"method"];
        [dict setObject:[PlayIWrapper robotToDict:robot]      forKey:@"robot" ];
        [dict setObject:idKey forKey:@"sequenceId" ];
