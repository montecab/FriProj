...
public abstract class piBotBase {
	...
	public delegate void onBotStateDelegate(piBotBase bot, SimpleJSON.JSONClass jsComponents);
	public static event onBotStateDelegate onBotState;
	...
	*/
	...
	public void handleState(SimpleJSON.JSONClass jsComponents) {
		foreach(string key in jsComponents.Keys) {
			piBotComponentBase component = validateComponentID(int.Parse(key));
			if (component != null) {
				component.handleState(jsComponents[key].AsObject);
			}
		}

		if (onBotState != null) {
			onBotState(this, jsComponents);
		}	
	}
