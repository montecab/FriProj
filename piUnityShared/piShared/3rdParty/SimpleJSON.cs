...
namespace WW.SimpleJSON
{
    ...
    {
        private const bool allowEmptyValues = false;
        ...
        public virtual long AsLong
        {
            ...
            {
                Value = value.ToString();
                valueType = JSONBinaryTag.LongValue;
            }
        }
...
// oxe : the original code here would not add the value token if it was empty.
//       this means that {"foo":""} would parse as an empty dictionary instead of a dictionary w/ a key w/ an empty value.
//       changing this.
                        if ((Token != "") || allowEmptyValues)
                        {
                            TokenName = TokenName.Trim();
                            if (ctx is JSONArray)
                                ctx.Add(Token);
        ...
        public void SaveToCompressedStream(System.IO.Stream aData)
        {
            throw new Exception("Can't use compressed functions. You need include the SharpZipLib and uncomment the define at the top of WW.SimpleJSON");
        }
        public void SaveToCompressedFile(string aFileName)
        {
            throw new Exception("Can't use compressed functions. You need include the SharpZipLib and uncomment the define at the top of WW.SimpleJSON");
    ...
    public class JSONClass : JSONNode, IEnumerable
    {
        ...
        public bool ContainsKey<T>(T aKey) {
          return m_Dict.ContainsKey(aKey.ToString());
        }

        public bool SafeGet<T>(T aKey, bool defVal)
        {
          string sKey = aKey.ToString();
          if (ContainsKey(sKey)) {
            return this[sKey].AsBool;
          }
          else {
            return defVal;
          }
        ...
        public int SafeGet<T>(T aKey, int defVal)
        {
          string sKey = aKey.ToString();
          if (ContainsKey(sKey)) {
            return this[sKey].AsInt;
          }
          else {
            return defVal;
          }
        ...
        public long SafeGet<T>(T aKey, long defVal)
        {
          string sKey = aKey.ToString();
          if (ContainsKey(sKey)) {
            return this[sKey].AsLong;
          }
          else {
            return defVal;
          }
        ...
        public float SafeGet<T>(T aKey, float defVal)
        {
          string sKey = aKey.ToString();
          if (ContainsKey(sKey)) {
            return this[sKey].AsFloat;
          }
          else {
            return defVal;
          }
        ...
        public string SafeGet<T>(T aKey, string defVal)
        {
          string sKey = aKey.ToString();
          if (ContainsKey(sKey)) {
            return this[sKey];
        ...
        public void setIfNonnull(string key, string val) {
          if (val != null) {
            this[key] = val;
          }
        }

        public void setIfNonnull(string key, bool val) {
          this[key].AsBool = val;
        }

        public void setIfNonnull(string key, int val) {
            this[key].AsInt = val;
        }

        public void setIfNonnull(string key, long val) {
            this[key].AsLong = val;
        }

        public void setIfNonnull(string key, float val) {
            this[key].AsFloat = val;
        }

        public void setIfNonnull(string key, JSONClass val) {
          if (val != null) {
            this[key] = val;
          }
        }

        public void setIfNonnull(string key, JSONArray val) {
          if (val != null) {
            this[key] = val;
          }
        }
    ...
    public class JSONData : JSONNode
    {
        ...
        private bool isBooleanOrNumeric {
          get {
            float vFloat;
            bool  vBool;
            ...
            if (bool.TryParse(Value,out vBool)) {
              return true;
            }
            return false;
            }
...
//            oxe - just NotSupportedException sure this is safe to do because strings such as "1.0" will be converted to floats.        
