        ...
        void Start ()
        {
            ...
            switch (mode)
            {
                ...
                case Mode.SEEK:
                    ...
                    OnDraged = new Action<float>((f) => { player.SeekTo(f); });
                    break;
                case Mode.VOLUME:
                    _slider = GetComponent<Slider>();
	    ...
	    void Update () {
            ...
            switch (mode)
            {
                case Mode.SEEK:
                    ...
                    {                     
                        try {
                            _slider.value = Mathf.Clamp01(System.Convert.ToSingle(player.GetCurrentPosition()));
                        }
                        catch (Exception)
                        {
                ...
                case Mode.TIME:
                    //_Update();
                    if (OnUpdateTime != null) OnUpdateTime(player.GetCurrentPosition(), player.GetDuration());
                    break;
                case Mode.PATH:
