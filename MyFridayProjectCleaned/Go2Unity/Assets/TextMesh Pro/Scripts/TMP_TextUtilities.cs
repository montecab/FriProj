...
namespace TMPro
{
    ...
    public static class TMP_TextUtilities
    {
        ...
        public static string ReplaceUnsupportedCharacters (string source, string replacement = null)
        {
          string result = string.Empty;
          if(!string.IsNullOrEmpty(source)){
            for (int i = 0; i < source.Length; i++) {
              if (HasUnsupportedCharacters (source [i].ToString ())) {
        ...
        public static bool HasUnsupportedCharacters (string source)
        {
          TMP_Glyph glyph;
          TMP_FontAsset m_currentFontAsset = TMP_Settings.defaultFontAsset;
          TMP_FontAsset tempFontAsset;
          if (m_currentFontAsset != null && !string.IsNullOrEmpty(source)) {
            for (int i = 0; i < source.Length; i++) {
              glyph = null;
              if (m_currentFontAsset.characterDictionary.TryGetValue (source [i], out glyph) == false) {
                ...
                if (TMP_Settings.fallbackFontAssets != null && TMP_Settings.fallbackFontAssets.Count > 0) {
                  ...
                  for (int y = 0; y < TMP_Settings.fallbackFontAssets.Count; y++) {
                    ...
                    if (tempFontAsset.characterDictionary.TryGetValue (source [i], out glyph)) {
                      m_currentFontAsset = tempFontAsset;
                      break;
                    }
                  }
                }
                // Since the missing character is still unavailable return true
                if (glyph == null) {
                  return true;
                }
              } 
