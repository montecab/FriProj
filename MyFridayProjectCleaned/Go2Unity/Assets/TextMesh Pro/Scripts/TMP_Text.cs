...
namespace TMPro
{
    ...
    public class TMP_Text : MaskableGraphic
    {
        ...
        /// <summary>
        /// Gets the literal text without looking up in the localization table.
        /// </summary>
        ...
        public string text {
          get { return m_text; }
          set { 
            if (!m_isTranslated && Application.isPlaying && gameObject.tag!=wwLocaManager.DO_NOT_TRANSLATE) {
              bool isFound;
              m_text = Turing.trDataManager.Instance.LocaManager.Format(value, out isFound);
              m_isTranslated = true;
              if (!isFound) {
                Debug.LogError(string.Format("string not found: \"{0}\" in UIPath: {1}", value, piUnityUtils.pathToTransform(transform)));
              }
              else if(Turing.trMultivariate.Instance.getOptionValue(Turing.trMultivariate.trAppOption.DEBUG_TRANSLATION_MODE)==Turing.trMultivariate.trAppOptionValue.YES){
                m_text = Turing.trMultivariate.DEBUG_TRANSLATION_TOKEN;
              }
            } else {
              m_text = value;
            }
