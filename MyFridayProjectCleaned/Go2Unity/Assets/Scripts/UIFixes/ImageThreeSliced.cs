...
namespace UnityEngine.UI
{
  ...
  public class ImageThreeSliced : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
  {

    #if UNITY_EDITOR
    [UnityEditor.MenuItem("GameObject/UI/WW/ThreeSlicedImage")]
    static void CreateImageThreeSliced(){
      GameObject image = Instantiate(UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/piShared/piPrefabs/UI/ThreeSlicedImage.prefab", typeof(GameObject) )) as GameObject;
      if(image != null){
        image.transform.SetParent( UnityEditor.Selection.activeTransform, false);
        image.gameObject.name = "ThreeSlicedImage";
      }
      else{
        Debug.LogError( "Can't find the ThreeSlicedImage object ");    
      }
    }
    #endif
    ...
    public enum ThreeSliceDirection{
      Horizontal,
      Vertical
    }
    ...
    public Sprite sprite { get { return m_Sprite; } set { if (wwUtility.SetClass(ref m_Sprite, value)) SetAllDirty(); } }

    [SerializeField] private ThreeSliceDirection m_Direction = ThreeSliceDirection.Horizontal;
    public ThreeSliceDirection direction { get { return m_Direction; } set { if (wwUtility.SetStruct(ref m_Direction, value)) SetVerticesDirty(); } }

    [NonSerialized]
    private Sprite m_OverrideSprite;
    public Sprite overrideSprite { get { return m_OverrideSprite == null ? sprite : m_OverrideSprite; } set { if (wwUtility.SetClass(ref m_OverrideSprite, value)) SetAllDirty(); } }

    /// How the Image is drawn.
    [NonSerialized] private Type m_Type = Type.Sliced;
    public Type type { get { return m_Type; } set { if (wwUtility.SetStruct(ref m_Type, value)) SetVerticesDirty(); } }

    [NonSerialized] private bool m_PreserveAspect = false;
    public bool preserveAspect { get { return m_PreserveAspect; } set { if (wwUtility.SetStruct(ref m_PreserveAspect, value)) SetVerticesDirty(); } }

    [NonSerialized] private bool m_FillCenter = true;
    public bool fillCenter { get { return m_FillCenter; } set { if (wwUtility.SetStruct(ref m_FillCenter, value)) SetVerticesDirty(); } }

    /// Filling method for filled sprites.
    [NonSerialized] private FillMethod m_FillMethod = FillMethod.Radial360;
    public FillMethod fillMethod { get { return m_FillMethod; } set { if (wwUtility.SetStruct(ref m_FillMethod, value)) {SetVerticesDirty(); m_FillOrigin = 0; } } }

    /// Amount of the Image shown. 0-1 range with 0 being nothing shown, and 1 being the full Image.
    [Range(0, 1)]
    [NonSerialized] private float m_FillAmount = 1.0f;
    public float fillAmount { get { return m_FillAmount; } set { if (wwUtility.SetStruct(ref m_FillAmount, Mathf.Clamp01(value))) SetVerticesDirty(); } }

    /// Whether the Image should be filled clockwise (true) or counter-clockwise (false).
    [NonSerialized] private bool m_FillClockwise = true;
    public bool fillClockwise { get { return m_FillClockwise; } set { if (wwUtility.SetStruct(ref m_FillClockwise, value)) SetVerticesDirty(); } }

    /// Controls the origin point of the Fill process. Value means different things with each fill method.
    [NonSerialized] private int m_FillOrigin;
    public int fillOrigin { get { return m_FillOrigin; } set { if (wwUtility.SetStruct(ref m_FillOrigin, value)) SetVerticesDirty(); } }
    ...
    private void GenerateSlicedSprite(VertexHelper toFill)
    {
      ...
      //force other direction border to be zero
      if(m_Direction == ThreeSliceDirection.Horizontal){
        border.y = 0;
        border.w = 0;
      }
      else{
      ...
      float leftRatio = border.x /sprite.rect.height;
      float rightRatio = border.z /sprite.rect.height;
      float topRatio = border.y /sprite.rect.width;
      float bottomRatio = border.w /sprite.rect.width;
      ...
      float left = m_Direction == ThreeSliceDirection.Horizontal ? leftRatio * rect.height : border.x;
      float right = m_Direction == ThreeSliceDirection.Horizontal ? rightRatio* rect.height: border.z;
      float top = m_Direction == ThreeSliceDirection.Vertical ? topRatio * rect.width : border.y;
      float bottom = m_Direction == ThreeSliceDirection.Vertical ? bottomRatio * rect.width : border.w;

      s_VertScratch[0] = new Vector2(padding.x, padding.y);
      s_VertScratch[3] = new Vector2(rect.width - padding.z, rect.height - padding.w);

      s_VertScratch[1].x = left;
      s_VertScratch[1].y = top;
      s_VertScratch[2].x = rect.width - right;
      s_VertScratch[2].y = rect.height - bottom;

      //Debug.LogError("vert: 0:" +  s_VertScratch[0] + ", 1: " + s_VertScratch[1]+ ", 2: " + s_VertScratch[2]+ ", 3: " + s_VertScratch[3]);
      ...
      float innerX = m_Direction == ThreeSliceDirection.Horizontal ? inner.x : outer.x;
      float innerZ = m_Direction == ThreeSliceDirection.Horizontal ? inner.z : outer.z;
      float innerY = m_Direction == ThreeSliceDirection.Vertical ? inner.y : outer.y;
      float innerW = m_Direction == ThreeSliceDirection.Vertical ? inner.w : outer.w;

      s_UVScratch[0] = new Vector2(outer.x, outer.y);
      s_UVScratch[1] = new Vector2(innerX, innerY);
      s_UVScratch[2] = new Vector2(innerZ, innerW);
      s_UVScratch[3] = new Vector2(outer.z, outer.w);

      //Debug.LogError("uv: 0:" +  s_UVScratch[0] + ", 1: " + s_UVScratch[1]+ ", 2: " + s_UVScratch[2]+ ", 3: " + s_UVScratch[3]);
